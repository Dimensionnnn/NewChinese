...
function AnalyzePage() {
  ...
  const setAnalyze = (newContent) => {
    ...
    if (newContent === "") {
      ...
      //选择公开
      if (window.confirm("提交入analyze")) {
        // 此处要根据修改后tiny内的值进行修改，例如下面text所述
        const new_id = nanoid();
        client.index("analyze").addDocuments([
          {
            id: new_id,
      ...
      //选择私有
      if (window.confirm("虽然选了私有，但还是存入analyze，待讨论")) {
        const new_id = nanoid();
        client.index("analyze").addDocuments([
          {
            id: new_id,
  ...
  const handleEditorChange = () => {
    ...
    if (!value.replace(/(<([^>]+)>)/ig, '').replace(/&nbsp;/gi, "").match(/[\u4e00-\u9fa5]/g)) {
      setOpenAlert(true);
      return;
    }
    let numberOfWords = editorValue.current.plugins.wordcount.body.getWordCount();
    ...
    Promise.all([...value.replace(/(<([^>]+)>)/ig, '').replace(/&nbsp;/gi, "").match(/[\u4e00-\u9fa5]/g).join("")].map(word => searchForWords(word))).then((values) => {
      values.forEach(
        value => {
          copiedWordCount[value] ? copiedWordCount[value]++ : copiedWordCount[value] = 1;
        }
      )
    ...
    Promise.all([...value.replace(/(<([^>]+)>)/ig, '').replace(/&nbsp;/gi, "").match(/[\u4e00-\u9fa5]/g).join("")].map(word => searchForWordsLevel(word))).then((values) => {
      values.forEach(
        value => {
          copiedWordLevelCount[value] ? copiedWordLevelCount[value]++ : copiedWordLevelCount[value] = 1;
        }
      )
    ...
    getSplitWord(value.replace(/(<([^>]+)>)/ig, '').replace(/&nbsp;/gi, "").match(/[\u4e00-\u9fa5]/g).join("")).then(val => {
  ...
  const getPhrase = () => {
    ...
    Promise.all(groupOfPhrases.map(phrase => searchForPhrases(phrase))).then((values) => {
      values.forEach(
        value => {
          copiedPhraseCount[value] ? copiedPhraseCount[value]++ : copiedPhraseCount[value] = 1;
        }
      )
    ...
    Promise.all(groupOfPhrases.map(phrase => searchForPhrasesLevel(phrase))).then((values) => {
      values.forEach(
        value => {
          copiedPhraseLevelCount[value] ? copiedPhraseLevelCount[value]++ : copiedPhraseLevelCount[value] = 1;
        }
      )
            ...
            <Paper sx={{ height: 630 }} elevation={3}>
              <Container sx={{ display: "flex", justifyContent: 'center' }}>
                <Stack direction="row" divider={<Divider orientation="vertical" flexItem />} spacing={3}>
                  <Button onClick={() => setDisplayBasic(true)}>基本信息</Button>
                  <Button onClick={() => setDisplayBasic(true)}>下载分析结果</Button>
                  <Button onClick={() => setDisplayBasic(false)}>高阶信息</Button>
                </Stack>
              </Container>
              {displayBasic ?
                <Container>
                  <p><strong>HSK字表</strong> 总字数 {totalWordNumber} HSK大纲字数 {inHSKTableCount} {(100.0 * inHSKTableCount / totalWordNumber).toFixed(2)}%</p>
                  <Table>
                    <TableHead>
                      <TableRow>
                  ...
                  <Divider />
                  <p><strong>等级大纲字表</strong> 总字数 {totalWordNumber} 369级大纲字数 {inLevelTableCount} {(100.0 * inLevelTableCount / totalWordNumber).toFixed(2)}%</p>
                  <Table>
                    <TableHead>
                      <TableRow>
                  ...
                  <Divider />
                  <p><strong>HSK词表</strong> 总词数 {totalPhraseNumber} HSK大纲词数 {inHSKTablePhraseCount} {(100.0 * inHSKTablePhraseCount / totalPhraseNumber).toFixed(2)}%</p>
                  <Table>
                    <TableHead>
                      <TableRow>
                  ...
                  <Divider />
                  <p><strong>等级大纲词表</strong> 总词数 {totalPhraseNumber} 369级大纲词数 {inLevelTablePhraseCount} {(100.0 * inLevelTablePhraseCount / totalPhraseNumber).toFixed(2)}%</p>
                  <Table>
                    <TableHead>
                      <TableRow>
                    ...
                    <TableBody>
                      <TableRow>
                        {Object.entries(phraseLevelCount).map(([key, value]) => {
                          return (
                            <TableCell key={key}>
                              {value}
                            </TableCell>
